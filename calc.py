...
def operatons_rational(update, context):
    ...
    if user_choice in '+-/*':
        ...
        update.message.reply_text('Выберите действие еще раз \n+ - для сложения; \n- - для вычетания; \n* - для умножения; \n/ - для деления. \n' )
...
def complex_one(update, context):
    ...
    if ' ' in test and (test.replace(' ', '')).isdigit():
        ...
        update.message.reply_text('Введите Re и Im первого числа через ПРОБЕЛ')
